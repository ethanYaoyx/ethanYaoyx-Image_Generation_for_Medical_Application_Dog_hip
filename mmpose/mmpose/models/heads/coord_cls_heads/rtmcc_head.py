...
class RTMCCHead(BaseHead):
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        """
        feats = feats[-1]
        #print(f"Input to final_layer: {feats.shape}") 
        feats = self.final_layer(feats)  # -> B, K, H, W
        #print(f"Output from final_layer: {feats.shape}")
        # flatten the output heatmap
        feats = torch.flatten(feats, 2)
        #print(f"After flatten: {feats.shape}")
        feats = self.mlp(feats)  # -> B, K, hidden
        #print(f"After MLP: {feats.shape}") 
        feats = self.gau(feats)
        #print(f"After GAU: {feats.shape}")
        pred_x = self.cls_x(feats)
        pred_y = self.cls_y(feats)
        #print(f"Predictions: pred_x={pred_x.shape}, pred_y={pred_y.shape}")
